...
export async function checkForUpdates(currentVersion: string) {
  try {
    ...
    const { stdout } = await execPromise("npm view @jhangyu/claude-code-router version");
...
export async function performUpdate() {
  try {
    ...
    const { stdout, stderr } = await execPromise("npm update -g @jhangyu/claude-code-router");
