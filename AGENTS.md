...
Codex Router is a tool that routes Codex requests to different LLM providers. It uses a Monorepo architecture with four main packages:

- **cli** (`@jhangyu/claude-code-router`): Command-line tool providing the `ccr` command
...
- **Project-level routing**: Checks `~/.Codex/projects/<project-id>/Codex-router.json`
...
The project uses the `@jhangyu/llms` package (external dependency) to handle request/response transformations. Transformers adapt to different provider API differences:
...
Configuration file location: `~/.Codex-router/config.json`
...
ccr code       # Execute Codex command
...
Presets are stored in `~/.Codex-router/presets/<preset-name>/manifest.json`
...
```
cli → server → shared
server → @jhangyu/llms (core routing and transformation logic)
...
4. **Build tools**:
   ...
   - ui: Vite + TypeScript
5. **@jhangyu/llms**: This is an external dependency package providing the core server framework and transformer functionality, type definitions in `packages/server/src/types.d.ts`
